...
# Traing ratio of generator:discriminator
GDRATIO = [1, 2]
...
# OUTPUT_NAME = DATASET + "_generatedImages"
...
def buildDiscriminator():
    ...
    model.add(keras.layers.Conv2D(64, kernel_size=(3, 3), activation=LeakyReLU(alpha=0.2), input_shape=IMAGE_SHAPE, padding='same'))
    model.add(keras.layers.Dropout(.2))
    model.add(keras.layers.Conv2D(32, kernel_size=(3, 3), activation=LeakyReLU(alpha=0.2), padding='same'))
    model.add(keras.layers.Dropout(.2))
...
def buildGenerator():
    ...
    model.add(keras.layers.Conv2DTranspose(32, kernel_size=(3, 3), activation=LeakyReLU(alpha=0.2), padding='same'))
    model.add(BatchNormalization(momentum=0.8))
    model.add(keras.layers.Conv2DTranspose(64, kernel_size=(3, 3), activation=LeakyReLU(alpha=0.2), padding='same'))
    model.add(BatchNormalization(momentum=0.8))
    model.add(keras.layers.Conv2DTranspose(128, kernel_size=(3, 3), activation=LeakyReLU(alpha=0.2), padding='same'))
    model.add(BatchNormalization(momentum=0.8))
    model.add(Dense(IMAGE_SIZE, activation="tanh"))
    model.add(Reshape(IMAGE_SHAPE))
    # Creating a Keras Model out of the network
...
def buildGAN(images, epochs = 40000, batchSize = 32, loggingInterval = 0):
    ...
    for epoch in range(epochs):
        ...
        if loggingInterval > 0 and epoch % loggingInterval == 0:
            ...
            discriminatorLoss.append(advLoss[0])
            print("\tEpoch %d:" % epoch)
    ...
    epochsLabel = np.arange(1, len(generatorLoss) + 1 ,1)
    plt.plot(epochsLabel, generatorLoss, label="GeneratorLoss")
    plt.plot(epochsLabel, discriminatorLoss, label="DiscriminatorLoss")
